...
public class Main {
	...
	public static void main(String args[]) {
		...
		options.addOption(
				...
				"target",
				true,
				"desired target. dot for DOT file OR gexf for GEXF files OR  seepjar for SEEP runnable jar");
		...
		options.addOption("cp", "classpath", true,
				"the path to additional libraries and code used by the input program");
		...
		String sootClassPath = javaHome + "/lib/rt.jar:" + pathToSeepJar + ":"
				+ "../seep-worker/build/libs/seep-worker-0.1.jar" + ":./";
		String pathToSourceCode = pathToDriverFile + "/" + className;

		/**
		 * Parse input program source code. This stage performs operations at
		 * high level only
		 **/
		...
		PhaseOptions.v().setPhaseOption("tag.ln", "on"); // tell compiler to
															// include line
															// numbers
		...
		Map<String, InternalStateRepr> stateElements = Util
				.extractStateInformation(fieldsIterator);
		...
		MainExtractor mainMethodExporter = new MainExtractor();
		mainMethodExporter.extractWorkflows(methodIterator, c);
		List<String> workflows = MainExtractor.getWorkflowNames();

		log.info("Extracting workflows...OK");

		// TODO: Insert a module here that checks for illegal programs (lower
		// level than only parsing source code)
		...
		for (String methodName : workflows) {
			...
			LiveVariableAnalysis lva = LiveVariableAnalysis.getInstance(cfg); // compute
																				// livevariables
			// Perform TE boundary analysis
			TEBoundaryAnalysis oba = TEBoundaryAnalysis.getBoundaryAnalyzer(
					cfg, stateElements, sch, lva);
			List<TaskElementBuilder> sequentialTEList = oba.performTEAnalysis();
			List<OperatorBlock> partialSDG = PartialSDGBuilder.buildPartialSDG(
					sequentialTEList, workflowId);
